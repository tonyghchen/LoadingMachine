void DispActAir (short set)
{
	...
	if( CurCursor.ID == MAN_AIR_CURSOR ){
		...
		FILL_DISP_BUF(0);
		DispStrLine_q(EDIT_BOX_LX2+AIR_LOC_POS*ASC_CHAR_W[idxFout], EDIT_BOX_LY+2, YELLOW_USR, RED_USR, DispBuf);
	}
	else{
		char *buf  ;
		...
		DispStrLine_q(EDIT_BOX_LX2+AIR_LOC_POS*ASC_CHAR_W[idxFout],
		...
		if(!AirTblOn){
			...
			DispStrLine_q(EDIT_BOX_LX2+(AIR_LOC_POS-3)*ASC_CHAR_W[idxFout],
