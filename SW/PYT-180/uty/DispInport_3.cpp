...
void DispInport(void)
{
	if(flag_DispInport){
		...
		if(cnnt == 500){
			cnnt=0 ;
			int y=470-ASC_CHAR_H[idxFout]*5 ;
			int x=10 ;
...
//					PrintStringLine_q(x,y,(lw&0x1)?RED_USR:YELLOW_USR , (lw&0x1)?YELLOW_USR:RED_USR,buf) ;
	...
	if(flag_DispInport){
		...
		if(cnnt == 500){
			cnnt=0 ;
			int y=516-ASC_CHAR_H[idxFout]*5 ;
			int x=10 ;
			uint32 lb=PORT_CN1;
			uint32 mb=PORT_INM ; 
			uint32 hb=PORT_INH ;
			...
			if(l != lInport){
				...
				x += ASC_CHAR_W[idxFout]*4 ;
				PrintfInport(x,y,l&0x2,"ESTOP") ;
				x += ASC_CHAR_W[idxFout]*6 ;
				PrintfInport(x,y,l&0x4,"DOOR") ;
				x += ASC_CHAR_W[idxFout]*5 ;
			...
			if(!same){
				int y=556-ASC_CHAR_H[idxFout]*5 ;
				int x=10 ;
